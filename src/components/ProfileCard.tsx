...
export default function ProfileCard({ user }: ProfileCardProps) {
  return (
    <div className="absolute h-[600px] w-96 overflow-hidden rounded-3xl bg-white shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15),0_0_1px_0_rgba(0,0,0,0.2)] ring-1 ring-black/5 backdrop-blur-xl select-none">
        ...
        {user.photos && user.photos.length > 1 && (
          <div className="pointer-events-none mt-5 flex gap-2">
            {user.photos.slice(0, 3).map((photo, i) => (
              <div
                key={i}
                className="relative h-20 w-20 overflow-hidden rounded-2xl ring-1 ring-black/5"
              >
                <img
                  src={photo}
                  alt={`${user.name} ${i + 1}`}
